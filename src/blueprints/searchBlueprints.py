...
def search():
    ...
    raw_results = utils.search_springer(query, start, year_range, type, exclude)
...
def search_by_keyword(keyword):
    ...
    if literatureType and not utils.is_in_types(literatureType):
        return jsonify({"error": "Invalid literature type"}), 400
    raw_results = utils.search_springer_by_keyword(keyword, start, year_range, literatureType, exclude)
    formatted_results = utils.format_search_results(raw_results)
...
def search_by_doi():
    ...
    raw_results = utils.search_springer_by_doi(dois, start, year_range, type, exclude)
    formatted_results = utils.format_search_results(raw_results)
...
def search_by_isbn():
    ...
    raw_results = utils.search_springer_by_isbn_issn("isbn",isbn, start, year_range, type, exclude)
    formatted_results = utils.format_search_results(raw_results)
...
def search_by_issn():
    ...
    raw_results = utils.search_springer_by_isbn_issn("issn", issn, start, year_range, type, exclude)
    formatted_results = utils.format_search_results(raw_results)
